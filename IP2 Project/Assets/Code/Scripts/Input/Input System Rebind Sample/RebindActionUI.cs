...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        {
            get => m_OverrideActionLabel;
            set => m_OverrideActionLabel = value;
        }
        public string actionLabelString
        {
            get => m_ActionLabelString;
            set => m_ActionLabelString = value;
        }
        ...
        public TMPro.TMP_Text rebindPrompt
        {
            get => m_RebindText;
        ...
        public void ResetToDefault()
        {
            ...
            // Reset while preventing duplicates.
        ...
        // A reset function to prevent duplicates when resetting a keybind.
        private void ResetBinding(InputAction action, int bindingIndex)
        {
            ...
            {
                // If the binding is a composite, remove the overrides from all part bindings.
            ...
            // Check all actions to prevent rebinding to a duplicate.
            foreach (InputAction otherAction in action.actionMap.actions)
            {
                ...
                // Otherwise, check all bindings for duplicates
                for (int i = 0; i < otherAction.bindings.Count; i++)
                {
                    InputBinding binding = otherAction.bindings[i];

                    // If this binding is a duplicate of the original value of this binding, then set the other binding to this binding's current value before we reset it.
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            // Disable the action before use to prevent an error.
            action.Disable();

            // Configure the rebind.
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                .WithCancelingThrough("<Keyboard>/escape") // Cancel the rebinding when you press Escape on a Keyboard.
                .WithCancelingThrough("<Gamepad>/start") // Cancel the rebinding when you press Start on a Gamepad.
                .OnCancel(
                    operation =>
                    {
                        action.Enable();
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        if (CheckDuplicateBindings(action, bindingIndex, allCompositeParts))
                        {
                            ...
                            // Reattempt the Rebind Operation.
        ...
        private bool CheckDuplicateBindings(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            // Loop through all current bindings.
            foreach (InputBinding binding in action.actionMap.bindings)
            {
                // Ignore the binding that we are currently setting.
                if (binding.action == newBinding.action)
                    continue;

                // If two separate bindings share the same effective path, then they are duplicates and we should return true.
                if (binding.effectivePath == newBinding.effectivePath)
                    return true;
            }

            // If we are to search through all bindings in a composite binding:
            if (allCompositeParts)
            {
                // Loop through all bindings prior to this one.
                for (int i = 1; i < bindingIndex; i++)
                {
                    // If the binding has the same effective path as the new binding, then they are duplicates and we should return true.
                    if (action.bindings[i].effectivePath == newBinding.effectivePath)
                        return true;
                }
            }

            // Otherwise, we found no duplicates.
            return false;
        }
        ...
        [SerializeField]
        private TMPro.TMP_Text m_ActionLabel;

        [Tooltip("Text label that will receive the current, formatted binding string.")]
        [SerializeField]
        private TMPro.TMP_Text m_BindingText;
        ...
        [SerializeField]
        private TMPro.TMP_Text m_RebindText;
